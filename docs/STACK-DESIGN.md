...
The `spaces stack` command helps developers split messy WIP commits into clean, reviewable PRs using AI-assisted code analysis in a sandboxed environment.
...
```
┌─────────────────────────────────────────────────────────────────┐
│  spaces stack - Analyzing changes...                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
...
Persists stack relationships for future `spaces stack sync`.

```typescript
// Stored in ~/spaces/<project>/.stack-state.json
...
# Specify base branch
spaces stack --base develop

# Specify commit range
spaces stack --range HEAD~5..HEAD

# Non-interactive mode (for CI/scripts)
spaces stack --auto

# Just analyze, don't execute
spaces stack --dry-run

# Provide hints to AI
spaces stack --hint "The auth changes should be separate from the UI changes"
...
4. Add basic `spaces stack --dry-run` command
...
1. User can run `spaces stack` and see proposed commit groupings
...
7. Stack state is persisted for future `spaces stack sync`
